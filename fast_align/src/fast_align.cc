...
bool InitCommandLine(int argc, char** argv) {
  ...
  if(print_scores) out_prob_file = input + ".prob"; 
...
#pragma omp parallel for schedule(dynamic) reduction(+:emp_feat_,c0_,likelihood_)
  for (int line_idx = 0; line_idx < static_cast<int>(lines.size());
    ...
    for (unsigned j = 0; j < trg.size(); ++j) {
      ...
      if (final_iteration) {
      	unsigned print_id = d.Convert("--choose--");
      	if (print_id == src[0]) {
	        double max_p = -1;
	        double pi = -1;
	        int max_index = -1;
	        ...
	        if (max_index >= 0) {
	          ...
	          //  oss << ' ' << endl;
	          if (is_reverse)
	            oss << '\t' << j << '|' << (max_index - 1) << endl;
	          else
	            oss << '\t' << (max_index - 1) << '|' << j << endl;
	        }
	      }
      } else {
        if (use_null) {
...
int main(int argc, char** argv) {
  ...
  for (int iter = 0; iter < ITERATIONS; ++iter) {
    ...
    ifstream in(input.c_str());
	ofstream prob_out;
	if (final_iteration && print_scores) {
		prob_out.open(out_prob_file.c_str());
	}
    if (!in) {
      cerr << "Can't read " << input << endl;
      return 1;
      ...
      if (buffer.size() >= thread_buffer_size) {
        ...
        if (final_iteration) {
          for (const string& output : outputs) {
            if(print_scores) prob_out << output;
			else cout << output;
          }
        }
        buffer.clear();
