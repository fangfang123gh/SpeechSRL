...
add_arg("eval_steps",    type=int, default=2000,    help="多少步数评估一次")
add_arg("save_steps",    type=int, default=2000,    help="多少步数保存模型一次")
...
add_arg("num_train_epochs", type=int, default=50,      help="训练的轮数")
...
add_arg("lora_r", type=int, default=32,    help="梯度累积步数")
add_arg("lora_alpha", type=int, default=64,    help="梯度累积步数")
add_arg("lora_dropout", type=float, default=0.05,    help="梯度累积步数")
...
if args.resume_from_checkpoint:
    ...
    if args.tune_whisper == 'none':
        ...
        if args.use_adalora:
            ...
            config = LoraConfig(r=args.lora_r, lora_alpha=args.lora_alpha, target_modules=target_modules, lora_dropout=args.lora_dropout, bias="none")
...
training_args = \
    Seq2SeqTrainingArguments(output_dir=output_dir,  # 保存检查点和意志的目录
                             ...
                             evaluation_strategy="steps",  # 指定按照步数评
                             num_train_epochs=args.num_train_epochs,  # 微调训练轮数
                             save_strategy="steps",  # 指定按照步数保存检查估模型
