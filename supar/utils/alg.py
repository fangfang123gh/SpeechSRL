...
def chuliu_edmonds(s):
    ...
    def contract(s):
        ...
        #                                                    s(cycle) = sum(s(a(v)->v))
        s_head = s[cycle][:, noncycle] - s_cycle.view(-1, 1).contiguous() + s_cycle.sum()
...
def eisner(scores, mask, multiroot=False):
    ...
    for w in range(1, seq_len):
        ...
        # [batch_size, n, w]
        il = ir = ilr.permute(2, 0, 1).contiguous()
        ...
        cl_span, cl_path = cl.permute(2, 0, 1).contiguous().max(-1)
        s_c.diagonal(-w).copy_(cl_span)
        p_c.diagonal(-w).copy_(cl_path + starts)
        # C(i->j) = max(I(i->r) + C(r->j)), i < r <= j
        cr = stripe(s_i, n, w, (0, 1)) + stripe(s_c, n, w, (1, w), 0)
        cr_span, cr_path = cr.permute(2, 0, 1).contiguous().max(-1)
...
def eisner2o(scores, mask, multiroot=False):
    ...
    for w in range(1, seq_len):
        ...
        il_span, il_path = il.permute(2, 0, 1).contiguous().max(-1)
        ...
        ir[:, 0] = ir0.squeeze(1)
        ir_span, ir_path = ir.permute(2, 0, 1).contiguous().max(-1)
        s_i.diagonal(w).copy_(ir_span + s_arc.diagonal(w))
        p_i.diagonal(w).copy_(ir_path + starts)

        # [n, w, batch_size]
        slr = stripe(s_c, n, w) + stripe(s_c, n, w, (w, 1))
        slr_span, slr_path = slr.permute(2, 0, 1).contiguous().max(-1)
    ...
    preds = []
    p_i = p_i.permute(2, 0, 1).contiguous().cpu()
    p_s = p_s.permute(2, 0, 1).contiguous().cpu()
    p_c = p_c.permute(2, 0, 1).contiguous().cpu()
    for i, length in enumerate(lens.tolist()):
        heads = p_c.new_zeros(length + 1, dtype=torch.long)
...
def cky(scores, mask):
    ...
    lens = mask[:, 0].sum(-1)
    scores = scores.permute(1, 2, 3, 0).contiguous()
    ...
    for w in range(1, seq_len):
        ...
        # [batch_size, n, w]
        s_s = s_s.permute(2, 0, 1).contiguous()
        # [batch_size, n]
        s_s, p = s_s.max(-1)
    ...
    p_s = p_s.permute(2, 0, 1).contiguous().tolist()
    p_l = p_l.permute(2, 0, 1).contiguous().tolist()
