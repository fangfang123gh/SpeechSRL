...
class BiaffineSemanticRoleLabelingModel(Model):
    ...
    def forward(self, words, feats=None, edges=None, if_prd=None):
        ...
        s_edge = self.edge_attn(edge_d, edge_h).permute(0, 2, 3, 1).contiguous()
        if(not self.args.split):
            ...
            if(edges != None):
                ...
                if_prd = if_prd.unsqueeze(-1).expand(-1, -1, label_d.shape[-1]).contiguous()
                label_d = label_d.masked_scatter(if_prd, prd_d)
                label_h = label_h.masked_scatter(if_prd, prd_h)

        # [batch_size, seq_len, seq_len, n_labels]
        s_label = self.label_attn(label_d, label_h).permute(0, 2, 3, 1).contiguous()
    ...
    def viterbi_decode3(self, s_edge, s_label, strans, trans, mask, mask2, B_idxs, I_idxs, prd_idx):
        ...
        #[batch_size, seq_len, seq_len, raw_label_num]
        weight1 = p_edge[..., 1].unsqueeze(-1).expand(-1, -1, -1, raw_label_num).contiguous()
        label_probs = weight1 * p_label
        # [batch_size, seq_len, seq_len, 2]
        weight2 = p_edge[..., 0].unsqueeze(-1).expand(-1, -1, -1, 2).contiguous()
        ...
        emit = pred_scores.transpose(0, 1).contiguous()
        ...
        label_preds = label_preds.transpose(1, 2).contiguous()
        # pdb.set_trace()
        label_preds = label_preds.masked_scatter(pred_mask.unsqueeze(-1).expand(-1, -1, seq_len_all).contiguous(), preds)
        label_preds = label_preds.transpose(1, 2).contiguous()
...
class VISemanticRoleLabelingModel(BiaffineSemanticRoleLabelingModel):
    ...
    def forward(self, words, feats=None, if_prd=None, bert_embeddings=None):
        r"""
        Args:
            words (~torch.LongTensor): ``[batch_size, seq_len]``.
                Word indices.
        ...
        """
        x = self.encode(words, feats, if_prd, bert_embeddings=bert_embeddings)
        edge_d = self.mlp_edge_d(x)
        edge_h = self.mlp_edge_h(x)
        ...
        s_edge = self.edge_attn(edge_d, edge_h).contiguous()
        # [batch_size, seq_len, seq_len, seq_len], (d->h->s)
        s_sib = self.sib_attn(pair_d, pair_d, pair_h)
        s_sib = (s_sib.triu() + s_sib.triu(1).transpose(-1, -2)).contiguous().permute(
            0, 3, 1, 2).contiguous()
        # [batch_size, seq_len, seq_len, seq_len], (d->h->c)
        s_cop = self.cop_attn(pair_h, pair_d, pair_h).permute(0, 3, 1, 2).contiguous()
        s_cop = s_cop.triu() + s_cop.triu(1).transpose(-1, -2).contiguous()
        # [batch_size, seq_len, seq_len, seq_len], (d->h->g)
        s_grd = self.grd_attn(pair_g, pair_d, pair_h).permute(0, 3, 1, 2).contiguous()
        # [batch_size, seq_len, seq_len, n_labels]
    ...
    def loss(self, s_edge, s_sib, s_cop, s_grd, x, labels, mask, if_pred=False):
        ...
        if(if_pred):
            ...
            edge_mask = labels.ge(0) & mask
            # print('edge_mask', edge_mask.is_contiguous())
