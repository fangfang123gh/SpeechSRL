...
class TransformerEmbedding(nn.Module):
    ...
    def forward(self, subwords, subwords_embed=None):
        r"""
        Args:
        ...
        if subwords_embed is not None:
            # bert_mask = (subwords_embed[..., -1] != self.pad_index).float()
            bert = self.bert(inputs_embeds=subwords_embed[:, :self.max_len, :], attention_mask=bert_mask[:, :self.max_len].float())[-1]
        else:
