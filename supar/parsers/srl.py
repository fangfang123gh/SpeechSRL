        ...
        for words, *feats, labels in progress_bar(loader):
            ...
            if(not self.args.vtb):
                ...
                if self.args.given_prd:
                    ...
                    prd_mask_2 = prd_mask.unsqueeze(-1).expand(-1, -1, seq_len).contiguous().transpose(1, 2).contiguous().clone()
                    prd_mask_2[:, :, 0] = 1
...
class VISemanticRoleLabelingParser(BiaffineSemanticRoleLabelingParser):
    ...
    def build_spans(self, label_pred, task='05'):
        ...
        if(task != '09'): 
            ...
            back_mask = pred_mask.unsqueeze(-1).expand(-1, -1, seq_len).contiguous().unsqueeze(-1).expand(-1, -1, -1,seq_len).contiguous()
            ...
            label_pred[:, 0] = -1
            label_pred[:, :, 0] = -1
            label_pred = label_pred.transpose(1, 2).contiguous()
