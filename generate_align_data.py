...
from model_with_suda import process_label_with_aligner
        ...
        with torch.cuda.amp.autocast():
            with torch.no_grad():
                ...
                for i in range(250):
                    ...
                    preds = processor.tokenizer.batch_decode(logits, skip_special_tokens=True)
                    for batch_idx, pred in enumerate(preds):
                        if len(pred) == 0:
                            continue
                        bert_encode = tokenizer(list(pred), max_length=512, truncation=True, add_special_tokens=False)['input_ids']
                        filter_notChinese = ''
                        for i, token_id in enumerate(bert_encode):
                            if len(token_id) == 1:

                                filter_notChinese += pred[i]
                                
                            elif len(token_id) == 0:

                                filter_notChinese += pred[i]

                        if filter_notChinese not in samples[batch_idx]:
                            samples[batch_idx].append(filter_notChinese)

                for sample, decoded_label in zip(samples, decoded_labels):
                    if sample != '':
                        fout.write(json.dumps({'text': decoded_label, 'generate_text': sample}, ensure_ascii=False)+'\n')
